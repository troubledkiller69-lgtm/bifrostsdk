...
- **Streaming** — `sendStreamingCommand()` for long-running operations (dump, generate, analyze, analyze_export). Each payload carries a `stream` tag; the backend echoes it on every event it emits.

**Stream routing** (4.0.0): events route by their `stream` tag to per-operation channels — `dump-log`, `dump-progress`, `dump-complete`, `dump-error`, plus parallel families for generate/analyze. `analyze_export` reuses the analyze channels. Before this, every log/result fanned out to all operations, so one operation's completion could auto-navigate another page.
...
Same pattern for the `generate` and `analyze` operations (log/progress/error/complete each).
