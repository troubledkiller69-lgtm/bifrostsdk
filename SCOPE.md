...
Nine tier-1 known drivers in `drivers/drivers_list.json`: Intel `iqvw64e.sys` (bulk physical), Dell `dbutil_2_3.sys` (CVE-2021-21551), MSI `RTCore64.sys` (CVE-2019-16098, dword loop), Dell `WDTKernel.sys` (WHQL), Corsair `CorsairLLAccess64.sys` (WHQL, MMIO), Gigabyte `gdrv.sys` (4× CVE-2018-19320–23), plus wave-1: SIV `SIVX64.sys` (WHQL thru 2026, unblocklisted, scatter read + mapped write), ThrottleStop `ThrottleStop.sys` (CVE-2025-7771, qword loop), Lenovo `LnvMSRIO.sys` (CVE-2025-8061, struct R/W). Three tier-2 candidates with null IOCTLs. Wave-1 `.sys` files are NOT shipped (bring your own — sidecars in `drivers/byo/`); `iqvw64e.sys` + `RTCore64.sys` remain in-tree. `.sys` never touches git.
...
**BYO**: drop a `.sys` + a sidecar `.json` in `drivers/byo/` — `key`, `filename`, `service_name`, `device_path`, `strategy` (`intel`/`msi`/`wdt`/`corsair`/`gigabyte`/`siv`/`throttlestop`/`lenovo`, or `generic_bulk`/`generic_dword` with custom `ioctl_read`/`ioctl_write`), `known_hashes` (empty = skip check), `description`. The mapper merges repo `drivers/byo/`, frozen `exe/drivers/byo/`, and single-file `drivers.json` lists. `example_generic_bulk.json` is the template; `sivx64.json`/`throttlestop.json`/`lenovo.json` are ready to use once you drop in the `.sys`.
...
- **AC detect (backend only, no page)** — `ac_detect` still scans for EAC/BattlEye/Vanguard/RICOCHET/nProtect (processes + services + drivers) over the bridge, but the Monitor page is gone. Transport picking is manual via the stealth cycler.
